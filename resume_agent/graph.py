...
def plan_edits(state: TailorState) -> dict:
    ...
    # ---- 1. Select top 2 relevant projects ----
    ...
    for proj in all_projects:
        ...
        score = sum(1 for k in jd_keywords if k in text)
        if score > 0:
    ...
    # Filter skills: STRICT - only include if clearly mentioned in JD
    for category, skills in profile_skills.items():
        ...
        for skill in skills:
            ...
            # Check if skill is explicitly mentioned in JD keywords
            is_in_jd = any(
            ...
            # Also check against the full job description text for exact matches
            jd_text_lower = state.job_description.lower() if state.job_description else ""
            is_exact_match = skill_lower in jd_text_lower
            
            # Only add if explicitly relevant to JD
            if is_in_jd or is_exact_match:
                skills_by_category[mapped_category].add(skill)

    # Convert sets to sorted lists
    skills_by_category = {k: sorted(v) for k, v in skills_by_category.items() if v}
...
def apply_edits(state: TailorState) -> dict:
    ...
    for rel, content in state.resume_files.items():
        ...
        try:
            ...
            skills_template = tex.load_template("skills_format.tex", state.resume_root_dir)
            ...
            generated_skills_latex = tools.call_llm(
                system_prompt=prompts.GENERATE_SKILLS_SYSTEM,
            ...
            state.changes.append("Generated skills section using LLM with custom template")
            ...
            state.changes.append("Used fallback method for skills (LLM generation failed)")
    ...
    for rel, content in state.resume_files.items():
        ...
        try:
            ...
            projects_template = tex.load_template("projects_format.tex", state.resume_root_dir)
            ...
            for i, proj in enumerate(selected_projects, 1):
                ...
                projects_data_str += f"Tech Stack: {', '.join(proj.get('tech_stack', []))}\n"
            ...
            user_prompt = prompts.GENERATE_PROJECTS_USER.format(
                job_description=state.job_description[:1000] if state.job_description else "",  # Limit JD length
                job_title=job_title,
                jd_keywords=jd_keywords_str,
            ...
            generated_projects_latex = tools.call_llm(
                system_prompt=prompts.GENERATE_PROJECTS_SYSTEM,
                user_prompt=user_prompt
            )
            ...
            for proj in selected_projects:
                state.changes.append(f"Generated project '{proj['name']}' using LLM with custom template")
...
def compile_pdf(state: TailorState) -> dict:
    ...
    logs = compile_module.compile_latex(
        state.output_dir, state.main_tex, state.output_pdf_path
    )
    return {"compile_logs": logs}
